...
trawls <- read_csv("../../../Shelf_Evolution/Data/FishGlob/FishGlob_public_std_clean.csv") %>% 
  ...
  #full_join(LME_names) %>% 
  # Testing mesopelagic depth range
  drop_na(longitude, latitude, Latitude, Longitude) %>% 
  filter(year > 1998 & depth > 199 & depth < 1501) %>%
...
# FOR THIS DEPTH RANGE, TOO FEW SAMPLES TO RAREFY BY SUMMARY STATS AS IN MAIN TEXT
...
#sf::st_write(obj = map_clusts,paste0("../Data/",method, ,"_" ,depth,"_Bioregions_",res,".shp"))
...
ggsave(paste0("../Figures/PC_", method, "_",depth, "_",cutoff,"_",res,".png"), width = 17, height = 15, dpi = 600, units = "cm")

write_sf(map_clusts_PC, paste0("../Data/",method, "_", depth,"_", cutoff,"_Bioregions_",res,".shp"))
